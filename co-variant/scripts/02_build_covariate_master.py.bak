...
def derive_physical_activity(lifestyle):
    """Return IPAQ-style average MET-hours/day from six HPP fields.

    The duration question is conditionally skipped when the corresponding
    number of active days is zero.  Such a missing duration is therefore a
    structural zero, not missing physical-activity data.  A missing day value,
    or a missing duration after reporting one or more days, remains missing.
    """

    def domain_contribution(days_column, minutes_column, met_weight):
        raw_days = pd.to_numeric(lifestyle[days_column], errors="coerce")
        days = _numeric(lifestyle[days_column], minimum=0, maximum=7)
        minutes = _numeric(lifestyle[minutes_column], minimum=0, maximum=1440)

        zero_days = raw_days.eq(0)
        complete = zero_days | (days.gt(0) & minutes.notna())
        contribution = met_weight * days * minutes
        contribution.loc[zero_days] = 0.0
        return contribution.where(complete), complete

    walking, walking_complete = domain_contribution(
        "activity_walking_10min_days_weekly",
        "activity_walking_minutes_daily",
        3.3,
    )
    moderate, moderate_complete = domain_contribution(
        "activity_moderate_days_weekly",
        "activity_moderate_minutes_daily",
        4.0,
    )
    vigorous, vigorous_complete = domain_contribution(
        "activity_vigorous_days_weekly",
        "activity_vigorous_minutes_daily",
        8.0,
    )
    complete = walking_complete & moderate_complete & vigorous_complete
    result = (walking + moderate + vigorous) / 60.0 / 7.0
...
def build_covariate_master(
    ...
    lifestyle_piece["physical_activity_met_h_day"] = derive_physical_activity(
        lifestyle_piece
    )
    ...
    family_piece["family_history_diabetes"] = _family_count_flag(
        family_piece,
