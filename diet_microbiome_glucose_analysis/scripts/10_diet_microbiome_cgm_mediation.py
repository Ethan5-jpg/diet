...
-----------------------
The formal Diet-CGM cohort contains both eligible and ineligible participants.
This corrected version explicitly applies the same diabetes/A10 eligibility
flag used by the Diet->CGM models before any mediation fitting.

Default:
    --cohort-mode primary
    uses primary_cgm_analysis_eligible.

Sensitivity:
    --cohort-mode strict
    uses strict_known_nondiabetes_a10_eligible.
...
def prepare_cohort_frame(cohort, pid_col, score_map, outcome_map):
    # IMPORTANT: retain the formal cohort eligibility flags. The Diet->CGM
    # Model 0/2/3 pipeline explicitly filters on primary_cgm_analysis_eligible;
    # mediation must use the same participant rule rather than merely reading
    # score/outcome values from the formal cohort.
    eligibility_flags = [
        "primary_cgm_analysis_eligible",
        "strict_known_nondiabetes_a10_eligible",
        "known_diabetes_or_a10_excluded",
        "diabetes_a10_status_known",
    ]

    keep = [pid_col] + list(score_map.values()) + list(outcome_map.values())
    keep += [c for c in eligibility_flags if c in cohort.columns]
...
def apply_cohort_eligibility_filter(cohort, cohort_mode):
    """
    Apply the SAME diabetes/A10 eligibility logic used by the formal Diet->CGM
    models.

    primary:
        primary_cgm_analysis_eligible == True
        (explicit diabetes/A10 positives excluded; unknown status retained)

    strict:
        strict_known_nondiabetes_a10_eligible == True
        (only explicitly known non-diabetes/non-A10 participants retained)
    """
    if cohort_mode == "primary":
        flag = "primary_cgm_analysis_eligible"
    elif cohort_mode == "strict":
    ...
    if flag not in cohort.columns:
        raise RuntimeError(
            f"Formal cohort is missing required eligibility flag: {flag}\n"
            "Rebuild 00_current4_diet_cgm_cohort.csv with "
            "00_build_formal_diet_cgm_cohort.py before running mediation."
        )

    mask = normalize_bool(cohort[flag])
    before = len(cohort)
    ...
    print()
    print("=" * 100)
    print("DIABETES / A10 COHORT ELIGIBILITY")
    print("=" * 100)
    print(f"Cohort mode: {cohort_mode}")
    print(f"Eligibility flag: {flag}")
    print(f"Formal cohort rows before eligibility filter: {before}")
    print(f"Rows retained after eligibility filter: {after}")
    print(f"Rows excluded by eligibility filter: {before - after}")

    if "known_diabetes_or_a10_excluded" in cohort.columns:
        explicit_excluded = normalize_bool(
            cohort["known_diabetes_or_a10_excluded"]
        )
        print(
            "Explicit known diabetes/A10 positives in formal cohort: "
            f"{int(explicit_excluded.sum())}"
        )

    if "diabetes_a10_status_known" in cohort.columns:
        status_known = normalize_bool(cohort["diabetes_a10_status_known"])
        print(
            "Unknown diabetes/A10 status in formal cohort: "
            f"{int((~status_known).sum())}"
        )

    if out.empty:
        raise RuntimeError(
            f"No participants remain after applying cohort mode: {cohort_mode}"
        )
...
def main():
    ...
    parser.add_argument(
        ...
        default="primary",
        help=(
            "Participant eligibility rule. 'primary' excludes explicit known "
            "diabetes/A10 positives while retaining unknown status, matching "
            "the primary Diet->CGM models. 'strict' retains only explicitly "
            "known non-diabetes/non-A10 participants."
        ),
    )
    ...
    print(f"Cohort eligibility mode: {args.cohort_mode}")
    print(
        "Candidate gate: "
        + (
    ...
    cohort = apply_cohort_eligibility_filter(
        cohort,
        args.cohort_mode,
    )
    ...
    # Corrected PRIMARY run keeps the canonical Step-10 filenames.
    # STRICT sensitivity receives its own filenames so it can never overwrite
    # the primary analysis.
    if args.cohort_mode == "primary":
        output_prefix = "10_mediation"
    else:
        output_prefix = "10_strict_mediation"

    model_out = OUT_MODELS / f"{output_prefix}_paths_model3.csv"
    summary_out = OUT_REPORTS / f"{output_prefix}_summary.csv"
    sig_out = OUT_REPORTS / f"{output_prefix}_significant.csv"
    tier1_sig_out = OUT_REPORTS / f"{output_prefix}_tier1_significant.csv"
    gate_out = OUT_REPORTS / f"{output_prefix}_diet_outcome_gate.csv"
    cov_out = OUT_REPORTS / f"{output_prefix}_covariate_audit.csv"
    txt_out = OUT_REPORTS / f"{output_prefix}_summary.txt"
