#!/usr/bin/env python3
"""
Step 18c — Paper-20 STRICT cross-sectional mediation-style analysis.

This is the formal STRICT mediation sensitivity run on the frozen Step17 robust bridge set.
...
Strict participant cohort
--------------------------
Uses the strict known-nondiabetes/A10 eligibility logic audited in Step18a.
...
def prepare_path_data(
    ...
    need = [
        "participant_id", "strict_eligible",
    ...
    ok = d["strict_eligible"].fillna(False).astype(bool)
...
def main() -> int:
    ...
    audit = audit.loc[
        audit["cohort_mode"].astype(str).eq("strict")
    ].copy()
    ...
    if audit.duplicated(keys).any():
        raise RuntimeError("Duplicate Step18a strict audit path")
    ...
    if not truthy(robust["audit_ready"]).all():
        raise RuntimeError(
            "Step18a contains strict path(s) not ready for mediation"
        )
    ...
    print("=== STEP 18c PAPER-20 STRICT MEDIATION ===")
    ...
    suffix = (
        f"_strict_debug{len(results)}_b{args.bootstrap}"
        if debug_subset else "_strict"
    )
    ...
    model_out = models_dir / "18c_strict_mediation_paths.csv"
    summary_out = reports / "18c_strict_mediation_context_summary.csv"
    sig_out = reports / "18c_strict_significant.csv"
    consistent_out = (
        reports / "18c_strict_direction_consistent_significant.csv"
    )
    recurrence_out = reports / "18c_strict_species_recurrence.csv"
    txt_out = reports / "18c_strict_mediation_summary.txt"
    ...
    lines = [
        "=== STEP 18c PAPER-20 STRICT MEDIATION SUMMARY ===",
    ...
    lines += [
        ...
        "- The full Step17 robust path set is reused unchanged for strict-cohort sensitivity.",
        "- Highest-priority bridge status is annotation only.",
        "- Strict-cohort BH-FDR is calculated within each frozen Diet x CGM mediator family; it is a sensitivity/robustness analysis, not a redefinition of primary discovery.",
