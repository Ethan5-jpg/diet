"""用其他参与者相同 food_id 的等权中位数填补 rEDIH 事件重量。"""
...
SCRIPT_VERSION = "2026-08-14-redih-weight-review-v1"
...
IMPUTATION_DETAIL_COLUMNS = [
    "imputation_method",
    "other_participant_donor_count",
    "other_event_donor_count",
    "other_participant_weight_min_g",
    "other_participant_weight_p25_g",
    "other_participant_weight_median_g",
    "other_participant_weight_p75_g",
    "other_participant_weight_max_g",
    "other_participant_weight_relative_iqr",
    "suggested_weight_g",
    "resolution_status",
    "resolved_weight_g",
    "review_notes",
...
def load_mapping(mapping_csv: Path) -> pd.DataFrame:
    ...
    if mapping["food_id"].isna().any() or mapping["food_id"].duplicated().any():
        raise ValueError("rEDIH 映射表的 food_id 必须完整且唯一。")
    invalid_status = ~mapping["mapping_status"].isin(
        {"mapped", "not_applicable", "unmapped_missing_labels"}
    )
    if invalid_status.any():
        raise ValueError("rEDIH 映射尚未冻结，不能准备重量填补表。")
    return mapping
...
def scan_missing_weight_events(
    ...
    for chunk_number, chunk in enumerate(reader, start=1):
        ...
        weight = pd.to_numeric(selected["weight_g"], errors="coerce")
        target = selected["mapping_status"].eq("mapped") & (
            weight.isna() | weight.lt(0)
        )
        if target.any():
            review = selected.loc[
                target,
        ...
        print(
            "缺重量扫描第 {} 块：累计目标事件 {:,} 条".format(
                chunk_number, metrics["mapped_missing_weight_events"]
            )
        )
    if not partials:
        return pd.DataFrame(
    ...
    if missing["source_event_row"].duplicated().any():
        raise ValueError("rEDIH 缺重量复核表出现重复 source_event_row。")
...
def collect_food_id_donors(
    ...
    for chunk_number, chunk in enumerate(reader, start=1):
        ...
        if selected.empty:
            continue
        selected["weight_g"] = pd.to_numeric(selected["weight_g"], errors="coerce")
        donor = selected.loc[selected["weight_g"].gt(0), columns].copy()
        if not donor.empty:
            partials.append(donor)
            donor_event_count += len(donor)
        print(
            "供体扫描第 {} 块：累计正重量供体事件 {:,} 条".format(
                chunk_number, donor_event_count
            )
        )
    if not partials:
        return pd.DataFrame(columns=columns)
...
def add_other_participant_median_imputations(
    ...
    """按其他参与者同 food_id 的个人中位数之中位数批准填补。"""
    validate_columns(missing.columns.tolist(), ["participant_id", "food_id"], "缺重量事件表")
    donor_columns = [
        "food_id",
        "participant_id",
        "donor_event_count",
    ...
    validate_columns(
        participant_food_donors.columns.tolist(), donor_columns, "参与者-food_id 供体表"
    )
    donor_groups = {
        food_id: group.copy()
        for food_id, group in participant_food_donors.groupby("food_id", sort=False)
    }
    rows: List[Dict[str, object]] = []
    for _, event in missing.iterrows():
        pool = donor_groups.get(event["food_id"])
        if pool is None:
            pool = pd.DataFrame(columns=donor_columns)
        else:
            pool = pool.loc[pool["participant_id"].ne(event["participant_id"])].copy()
    ...
    details = pd.DataFrame(
        rows,
        index=missing.index,
        columns=IMPUTATION_DETAIL_COLUMNS,
    )
...
def preserve_existing_resolutions(
    ...
    """保留既有人工批准值；旧 pending/unresolved 让位于当前规则。"""
    ...
    if invalid.any():
        raise ValueError("既有 rEDIH 缺重量复核表含无效 resolution_status。")
    if existing.duplicated(EVENT_KEY_COLUMNS).any():
        raise ValueError("既有 rEDIH 缺重量复核表存在重复事件键。")
    existing_resolved = pd.to_numeric(existing["resolved_weight_g"], errors="coerce")
    approved = existing["resolution_status"].eq("use_resolved_weight")
    if (approved & (existing_resolved.isna() | existing_resolved.le(0))).any():
    ...
    merged = review.merge(
        existing, on=EVENT_KEY_COLUMNS, how="left", validate="one_to_one"
    )
...
def build_qc(
    ...
    participant_complete = review.groupby("participant_id")["resolution_status"].apply(
        lambda values: values.eq("use_resolved_weight").all()
    )
    ...
    metrics: Dict[str, object] = {
        **scan_metrics,
        ...
        "unresolved_event_count": int((~approved).sum()),
        "participants_with_all_missing_events_imputed": int(participant_complete.sum()),
...
def prepare_redih_weight_review(
    input_csv: Path,
    mapping_csv: Path,
    output_dir: Path,
    cohort: str,
    ...
    donors = collect_food_id_donors(
        input_csv,
        set(missing["food_id"].dropna().astype(str)),
        cohort,
        research_stage,
        chunk_size,
    )
    donor_summary = build_participant_food_donors(donors)
    review = add_other_participant_median_imputations(missing, donor_summary)

    output_dir.mkdir(parents=True, exist_ok=True)
    review_path = output_dir / "redih_missing_weight_review.csv"
    donor_path = output_dir / "redih_missing_weight_food_id_donors.csv"
    qc_path = output_dir / "redih_missing_weight_qc.csv"
    review = preserve_existing_resolutions(review, review_path)
    qc = build_qc(review, scan_metrics, cohort, research_stage)
    review.to_csv(review_path, index=False, encoding="utf-8-sig")
    donor_summary.to_csv(donor_path, index=False, encoding="utf-8-sig")
    qc.to_csv(qc_path, index=False, encoding="utf-8-sig")

    approved = review["resolution_status"].eq("use_resolved_weight")
    print("-" * 72)
    print("脚本版本：{}".format(SCRIPT_VERSION))
    print("缺重量事件：{:,} 条".format(len(review)))
    print("受影响参与者：{:,} 人".format(review["participant_id"].nunique()))
    print("按其他参与者同 food_id 中位数填补：{:,} 条".format(int(approved.sum())))
    print("仍无法填补：{:,} 条".format(int((~approved).sum())))
    print("重量复核及 override 表：{}".format(review_path))
    print("参与者-food_id 供体统计：{}".format(donor_path))
...
def parse_args() -> object:
    parser = ArgumentParser(
        description="用其他参与者相同 food_id 的等权中位数填补 rEDIH 重量"
    )
