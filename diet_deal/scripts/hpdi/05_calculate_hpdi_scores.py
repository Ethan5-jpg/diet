"""计算18项 hPDI 原始总分、能量校正分、Z 分数和五分位。"""
...
SCRIPT_VERSION = "2026-08-14-hpdi-scores-v6"
...
IMPUTATION_COUNT_COLUMN = "total_weight_imputed_events"
IMPUTATION_FLAG_COLUMN = "hpdi_any_weight_imputed"
...
def load_intakes(input_csv: Path) -> pd.DataFrame:
    ...
    optional_imputation_columns = {
        IMPUTATION_COUNT_COLUMN,
        IMPUTATION_FLAG_COLUMN,
    }
    observed_optional_columns = optional_imputation_columns.intersection(
        data.columns
    )
    if (
        observed_optional_columns
        and observed_optional_columns != optional_imputation_columns
    ):
        missing_optional = sorted(
            optional_imputation_columns - observed_optional_columns
        )
        raise ValueError(
            "hPDI 摄入量表的填补来源字段不完整：" + ", ".join(missing_optional)
        )
    if observed_optional_columns:
        imputed_count = pd.to_numeric(
            data[IMPUTATION_COUNT_COLUMN], errors="coerce"
        )
        invalid_count = (
            imputed_count.isna()
            | imputed_count.lt(0)
            | (imputed_count % 1).ne(0)
        )
        if invalid_count.any():
            raise ValueError(
                f"{IMPUTATION_COUNT_COLUMN} 含缺失、负数或非整数。"
            )
        data[IMPUTATION_COUNT_COLUMN] = imputed_count.astype("int64")
        data[IMPUTATION_FLAG_COLUMN] = parse_boolean(
            data[IMPUTATION_FLAG_COLUMN], IMPUTATION_FLAG_COLUMN
        )
        inconsistent_flag = data[IMPUTATION_FLAG_COLUMN].ne(
            data[IMPUTATION_COUNT_COLUMN].gt(0)
        )
        if inconsistent_flag.any():
            raise ValueError("hPDI 填补事件数与填补标志不一致。")
    return data
...
def energy_adjust_score(
    ...
    standardized = pd.Series(
        pd.NA, index=raw_score.index, dtype="Float64"
    )
    ...
    adjusted_mean = float(adjusted_values.mean())
    adjusted_sd = float(adjusted_values.std(ddof=1))
    if not adjusted_sd > 0:
        raise ValueError("能量校正后的 hPDI 没有变异，无法进行 Z 标准化。")
    standardized_values = (adjusted_values - adjusted_mean) / adjusted_sd
    winsorized.loc[eligible] = score_winsorized
    adjusted.loc[eligible] = adjusted_values
    standardized.loc[eligible] = standardized_values
    parameters: Dict[str, object] = {
        ...
        "method": (
            ...
            "standardize adjusted score to mean 0 and sample SD 1"
        ),
        ...
        "winsorized_low_count": int(eligible_score.lt(lower).sum()),
        "winsorized_high_count": int(eligible_score.gt(upper).sum()),
        ...
        "adjusted_score_energy_correlation": float(
            adjusted_values.corr(eligible_energy)
        ),
        "z_score_standard_deviation_definition": "sample SD, ddof=1",
        "z_score_mean": float(standardized_values.mean()),
        "z_score_standard_deviation": float(
            standardized_values.std(ddof=1)
        ),
    }
    return winsorized, adjusted, standardized, parameters
...
def build_qc(
    ...
    rows = [
        ...
        (
            "overall",
            "",
            "participants_eligible_for_primary_score_after_weight_qc",
            int(scores["hpdi_weight_complete_for_primary_score"].sum()),
        ),
        (
            "overall",
            "",
            "participants_excluded_from_primary_score_for_incomplete_weight",
            int((~scores["hpdi_weight_complete_for_primary_score"]).sum()),
        ),
        (
            "overall",
        ...
        (
            ...
            int(scores["hpdi_score_energy_adjusted_z"].notna().sum()),
        ),
        (
            "overall",
            "",
            "hpdi_energy_adjusted_score_z_mean",
            scores["hpdi_score_energy_adjusted_z"].mean(),
        ),
        (
            "overall",
            "",
            "hpdi_energy_adjusted_score_z_standard_deviation",
            scores["hpdi_score_energy_adjusted_z"].std(ddof=1),
        ),
        (
            "overall",
            "",
            "hpdi_energy_adjusted_observed_quintile_count",
            int(scores["hpdi_energy_adjusted_quintile"].dropna().nunique()),
        ),
    ]
    for quintile in range(1, 6):
        rows.append(
            (
                "overall",
                "",
                "hpdi_energy_adjusted_quintile_{}_participant_count".format(
                    quintile
                ),
                int(
                    scores["hpdi_energy_adjusted_quintile"]
                    .eq(quintile)
                    .sum()
                ),
            )
        )
    if IMPUTATION_COUNT_COLUMN in scores.columns:
        rows.extend(
            [
                (
                    "overall",
                    ...
                    int(scores[IMPUTATION_COUNT_COLUMN].sum()),
                ),
                (
                    "overall",
                    "",
                    "participants_with_any_weight_imputation",
                    int(scores[IMPUTATION_FLAG_COLUMN].sum()),
                ),
            ]
        )
...
def calculate_hpdi_scores(
    ...
    if IMPUTATION_COUNT_COLUMN in intakes.columns:
        scores[IMPUTATION_COUNT_COLUMN] = intakes[IMPUTATION_COUNT_COLUMN]
        scores[IMPUTATION_FLAG_COLUMN] = intakes[IMPUTATION_FLAG_COLUMN]
    weight_complete_columns = [
        weight_complete_column(component) for component in COMPONENTS
    ]
    primary_weight_complete = intakes[weight_complete_columns].all(axis=1)
    scores["hpdi_weight_complete_for_primary_score"] = primary_weight_complete
    threshold_rows = []
    score_columns = []
    for component, direction in COMPONENT_DEFINITIONS:
        column = intake_column(component)
        observed_values = intakes[column]
        scoring_values = observed_values.where(primary_weight_complete)
        quintiles = assign_tie_safe_quintiles(scoring_values)
        component_scores = (
            quintiles if direction == "positive" else 6 - quintiles
        ).astype("Int64")
        scores[column] = observed_values
        ...
        threshold_rows.append(
            build_threshold_row(
                component, direction, scoring_values, quintiles
            )
        )
    ...
    (
        ...
        scores["hpdi_score_raw_18_90"], scores[ENERGY_COLUMN]
    )
    scores["hpdi_score_winsorized"] = winsorized
    scores["hpdi_score_energy_adjusted"] = adjusted
    scores["hpdi_score_energy_adjusted_z"] = standardized
    ...
    print(
        "最终 Z 分数（均值/样本SD）："
        f"{scores['hpdi_score_energy_adjusted_z'].mean():.6f}/"
        f"{scores['hpdi_score_energy_adjusted_z'].std(ddof=1):.6f}"
    )
...
def parse_args():
    parser = ArgumentParser(
        description="计算 hPDI 原始分、总能量残差校正分和最终 Z 分数"
    )
